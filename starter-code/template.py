...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_mini_text, gpt4o_mini_lat, gpt4o_mini_usage = call_openai(prompt, model=OPENAI_MINI_MODEL)
    ...
    gpt4o_mini_cost = (
        ...
        gpt4o_mini_usage["output_tokens"] * PRICING_1M_TOKENS["gpt-4o-mini"]["output"]
    ) / 1_000_000.0
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Append user message to history list of conversation turns
        history.append({"role": "user", "text": user_input})

        # Keep history limited to the last 3 turns (6 messages)
        history = history[-6:]

        # Format history as Content objects
        formatted_history = []
        for msg in history:
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for res in results:
        prompt = res["prompt"]
        for key, name in model_mapping:
            stats = res[key]
            raw_response = stats.get("response", "")
            # Replace newlines with space to keep markdown table formatting valid
            clean_text = raw_response.replace("\r", "").replace("\n", " ")
            if len(clean_text) > 50:
                clean_text = clean_text[:47] + "..."
            latency_str = f"{stats.get('latency', 0.0):.3f}s"
            tokens_str = f"{stats.get('input_tokens', 0)}/{stats.get('output_tokens', 0)}"
            cost_str = f"${stats.get('cost', 0.0):.6f}"
            lines.append(f"| {prompt} | {name} | {clean_text} | {latency_str} | {tokens_str} | {cost_str} |")
    return "\n".join(lines)
